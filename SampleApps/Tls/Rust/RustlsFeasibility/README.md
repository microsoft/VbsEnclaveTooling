...
The `tls_driver` module is a `no_std` + `alloc` TLS 1.3 client driver built on rustls' **unbuffered** connection API (rustls' buffered `ClientConnection`/`StreamOwned` are std-only and cannot run in the enclave). It drives the whole server-auth exchange — pinned-certificate handshake, one HTTP GET, bounded response read — behind a [`TlsTransport`] abstraction, and returns only a bounded, derived result. The same driver runs unchanged in the host harness (over a socket) and in the VTL1 enclave (over the `HostTcp*` EDL callbacks); only the transport differs.

The sibling `..\RustlsHostHarness` crate builds a std host-mode harness that runs this driver against the local test server before it is moved into a VTL1 Rust enclave.
