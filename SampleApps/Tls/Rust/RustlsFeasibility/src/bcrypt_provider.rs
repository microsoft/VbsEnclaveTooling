...
impl Hash for BcryptHash {
    ...
    fn hash(&self, data: &[u8]) -> HashOutput {
        ...
        if status < 0 {
            // A digest should never fail for a valid algorithm and buffer;
            // surface it loudly in debug rather than silently returning an empty
            // hash that masks the real fault as a downstream DecryptError.
            debug_assert!(status >= 0, "BCrypt hash failed: {status:#x}");
...
impl HmacKey for BcryptHmacKey {
    fn sign_concat(&self, first: &[u8], middle: &[&[u8]], last: &[u8]) -> Tag {
        ...
        if create_status < 0 {
            debug_assert!(create_status >= 0, "BCrypt HMAC create failed: {create_status:#x}");
            return Tag::new(&[]);
        }
        ...
        if status < 0 {
            debug_assert!(status >= 0, "BCrypt HMAC failed: {status:#x}");
            return Tag::new(&[]);
        }
...
impl ActiveKeyExchange for BcryptActiveKeyExchange {
    fn complete(self: Box<Self>, peer_pub_key: &[u8]) -> Result<SharedSecret, Error> {
        ...
        // BCRYPT_KDF_RAW_SECRET returns the shared secret in little-endian byte
        // order, but TLS uses the big-endian X coordinate. Reverse to big-endian
        // before (left-)padding to the fixed coordinate size, otherwise both
        // peers derive mismatched key schedules and every record fails to
...
impl ServerCertVerifier for PinnedServerVerifier {
    fn verify_server_cert(
        ...
        // Trust anchor: the exact pinned leaf certificate (SHA-256 of its DER).
        let actual = SHA256.hash(end_entity.as_ref());
        if actual.as_ref() != self.pinned_leaf_sha256 {
            return Err(Error::General("server certificate pin mismatch".into()));
        }

        // Identity: the leaf must also be valid for the requested name (SAN),
        // matching the C++ sample, which sets the hostname and rejects a
        // hostname/SAN mismatch even on a pin match. Time and issuer trust are
        // intentionally not checked here (the pin is the trust anchor).
        let parsed = webpki::EndEntityCert::try_from(end_entity)
            .map_err(|_| Error::General("server certificate could not be parsed".into()))?;
        parsed
            .verify_is_valid_for_subject_name(server_name)
            .map_err(|_| Error::General("server certificate name mismatch".into()))?;
