...
function Get-ServerCertificate {
    ...
    # Only fall back to sibling PEM files when the explicitly requested file does
    # not exist; never override an explicit path the caller asked for.
    if ((-not (Test-Path $fullPath)) -and (Test-Path $siblingPem) -and (Test-Path $siblingKey)) {
...
function Invoke-Connection {
    ...
    try {
        ...
        # Send a TLS close_notify before closing the socket so the enclave can
        # distinguish an authenticated clean shutdown from a bare TCP FIN (which
        # it treats as a truncated, untrusted response).
...
try {
    ...
    Write-Host "TLS sample test server listening on $Address`:$boundPort"
