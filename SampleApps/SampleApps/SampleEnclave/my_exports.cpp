...
HRESULT VbsEnclave::VTL1_Declarations::RunHelloSecuredEncryptionKeyExample_LoadEncryptionKey(_In_ const std::vector<std::uint8_t>& securedEncryptionKeyBytes, _In_ const std::wstring& dataToEncrypt, _In_ const bool isToBeEncrypted, _In_ const std::uint32_t activity_level, _In_ const std::wstring& logFilePath, _Out_  std::vector<std::uint8_t>& resealedEncryptionKeyBytes, _Out_  std::vector<std::uint8_t>& encryptedInputBytes, _Out_  std::vector<std::uint8_t>& tag, _Out_  std::wstring& decryptedData)
{
    using namespace veil::vtl1::vtl0_functions;
    const bool requireEnclaveOwnerIdMatchesHelloContainerSecureId = false;
    ...
    if (isToBeEncrypted)
    {
        ...
        auto [encryptedText, encryptTag] = veil::vtl1::crypto::encrypt(encryptionKey.get(), veil::vtl1::as_data_span(SOME_PLAIN_TEXT), veil::vtl1::crypto::zero_nonce);
        ...
        tag.assign(encryptTag.begin(), encryptTag.end());
    }
    else
    {
        ...
        decryptedData.assign(decryptedString.begin(), decryptedString.end());
    }
