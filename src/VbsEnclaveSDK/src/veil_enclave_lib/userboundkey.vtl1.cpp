...
using namespace veil::vtl1::debug;
...
veil_abi::Types::attestationReportAndSessionInfo userboundkey_get_attestation_report(_In_ const std::vector<std::uint8_t>& challenge)
{
    ...
    debug_print(L"DEBUG: userboundkey_get_attestation_report called - enclave function started");
    ...
    debug_print(challengeHex.c_str());
    ...
    debug_print(L"DEBUG: About to call InitializeUserBoundKeySession");
    ...
    debug_print(L"DEBUG: InitializeUserBoundKeySession completed successfully");
    ...
    debug_print(L"DEBUG: userboundkey_get_attestation_report returning successfully");
...
{
    debug_print(L"DEBUG: userboundkey_close_session called with session: 0x" + std::to_wstring(session_info));
...
namespace
{
        ...
        const wchar_t* functionName)
    {
        ...
        debug_print((std::wstring(functionName) + L" - Validating formatted key name security").c_str());
        debug_print((std::wstring(functionName) + L" - Expected suffix: " + expectedSuffix + L", Formatted: " + formattedKeyName).c_str());
    
        if (!formattedKeyName.ends_with(expectedSuffix))
        {
            debug_print((std::wstring(functionName) + L" - ERROR: Formatted key name does not end with expected pattern. Expected suffix: " + expectedSuffix + L", Formatted: " + formattedKeyName).c_str());
            debug_print((std::wstring(functionName) + L" - ERROR: This could be a security attack attempt with malicious key name").c_str());
            THROW_HR(E_ACCESSDENIED);
        }
     
        debug_print((std::wstring(functionName) + L" - Key name security validation passed (suffix check)").c_str());
    }
}
...
std::vector<uint8_t> GetEphemeralPublicKeyBytesFromBoundKeyBytes(std::span<const uint8_t> boundKeyBytes)
{
    ...
    debug_print((L"DEBUG: GetEphemeralPublicKeyBytesFromBoundKeyBytes - Starting with boundKeyBytes.size(): " + std::to_wstring(boundKeyBytes.size())).c_str());

    if (boundKeyBytes.size() < sizeof(uint32_t))
    {
        debug_print(L"ERROR: GetEphemeralPublicKeyBytesFromBoundKeyBytes - Bound key bytes too small - missing size field");
    ...
    debug_print((L"DEBUG: GetEphemeralPublicKeyBytesFromBoundKeyBytes - enclavePublicKeyBlobSize: " + std::to_wstring(enclavePublicKeyBlobSize)).c_str());

    // Validate that we have enough data for the full public key blob
    size_t remainingBytes = boundKeyBytes.size() - sizeof(uint32_t);
    debug_print((L"DEBUG: GetEphemeralPublicKeyBytesFromBoundKeyBytes - remainingBytes: " + std::to_wstring(remainingBytes)).c_str());

    if (remainingBytes < enclavePublicKeyBlobSize)
    {
        debug_print((L"ERROR: GetEphemeralPublicKeyBytesFromBoundKeyBytes - Insufficient data for public key blob. Need: " + std::to_wstring(enclavePublicKeyBlobSize) + L", Have: " + std::to_wstring(remainingBytes)).c_str());
    ...
    {
        debug_print((L"ERROR: GetEphemeralPublicKeyBytesFromBoundKeyBytes - Invalid public key blob size: " + std::to_wstring(enclavePublicKeyBlobSize) + L" (remainingBytes: " + std::to_wstring(remainingBytes) + L")").c_str());
    ...
    debug_print((L"DEBUG: GetEphemeralPublicKeyBytesFromBoundKeyBytes - Successfully extracted ephemeral public key, size: " + std::to_wstring(ephemeralPublicKey.size())).c_str());
...
{
    debug_print(L"DEBUG: create_user_bound_key - Function entered");
    ...
    try
    {
        ...
        debug_print(L"DEBUG: create_user_bound_key - Formatting key name");
        std::wstring formattedKeyName = veil::vtl1::implementation::userboundkey::callouts::userboundkey_format_key_name(keyName);
        debug_print((L"DEBUG: create_user_bound_key - formattedKeyName: " + formattedKeyName).c_str());
        ...
        debug_print(L"DEBUG: create_user_bound_key - Received credential and session info");
        ...
        debug_print((L"DEBUG: create_user_bound_key - credential pointer: 0x" + std::to_wstring(credentialWrapper.get())).c_str());
        ...
        debug_print(L"DEBUG: create_user_bound_key - userboundkey_get_authorization_context_from_credential completed");
        ...
        debug_print(L"DEBUG: create_user_bound_key - GetUserBoundKeyAuthContext completed");
        ...
        debug_print(L"DEBUG: create_user_bound_key - ValidateUserBoundKeyAuthContext completed");

        // USERKEY
        auto userkeyBytes = veil::vtl1::crypto::generate_symmetric_key_bytes();
        debug_print(L"DEBUG: create_user_bound_key - Generated user key bytes");

        // ENCRYPT USERKEY  
        wil::unique_process_heap_ptr<void> boundKey;
        uint32_t boundKeySize = 0;
        debug_print(L"DEBUG: create_user_bound_key - About to call ProtectUserBoundKey");
        ...
        debug_print((L"DEBUG: create_user_bound_key - ProtectUserBoundKey returned boundKey size: " + std::to_wstring(boundKeySize)).c_str());
        ...
        debug_print(L"DEBUG: create_user_bound_key - Function completed successfully");
    ...
    catch (const std::exception& e)
    {
        ...
        debug_print((L"ERROR: create_user_bound_key - Exception caught: " + werror_msg).c_str());
        throw; // Re-throw the exception
    }
    catch (...)
    {
        debug_print(L"ERROR: create_user_bound_key - Unknown exception caught");      
        throw; // Re-throw the exception
    }
}
    ...
    _Out_ bool& needsReseal)
{
    ...
    try
    {
        ...
        // UNSEAL
        debug_print(L"DEBUG: load_user_bound_key - About to call unseal_data");
        debug_print((L"DEBUG: load_user_bound_key - sealedBoundKeyBytes size: " + std::to_wstring(sealedBoundKeyBytes.size())).c_str());

        auto [boundKeyBytes, unsealingFlags] = veil::vtl1::crypto::unseal_data(sealedBoundKeyBytes);

        debug_print(L"DEBUG: load_user_bound_key - unseal_data completed successfully");     
        debug_print((L"DEBUG: load_user_bound_key - boundKeyBytes size: " + std::to_wstring(boundKeyBytes.size())).c_str());
        debug_print((L"DEBUG: load_user_bound_key - unsealingFlags: 0x" + std::to_wstring(unsealingFlags)).c_str());

        // Check if the sealing key is stale and data needs to be re-sealed
        if (unsealingFlags & ENCLAVE_UNSEAL_FLAG_STALE_KEY)
        {
            debug_print(L"ERROR: load_user_bound_key - Stale sealing key detected, operation failed");
            debug_print(L"ERROR: load_user_bound_key - Caller must handle re-sealing using reseal_user_bound_key function");
            debug_print(L"ERROR: load_user_bound_key - This is a security requirement to ensure data integrity");
        ...
        debug_print((L"DEBUG: load_user_bound_key - ephemeralPublicKeyBytes size: " + std::to_wstring(ephemeralPublicKeyBytes.size())).c_str());

        // Format the key name before establishing session
        debug_print(L"DEBUG: load_user_bound_key - Formatting key name");
        std::wstring formattedKeyName = veil::vtl1::implementation::userboundkey::callouts::userboundkey_format_key_name(keyName);
        debug_print((L"DEBUG: load_user_bound_key - formattedKeyName: " + formattedKeyName).c_str());
        ...
        debug_print(L"DEBUG: load_user_bound_key - About to call userboundkey_establish_session_for_load");
        ...
        debug_print(L"DEBUG: load_user_bound_key - userboundkey_establish_session_for_load completed");

        // Use RAII wrapper to prevent leaks
        unique_credential credentialWrapper(credentialAndSessionInfo.credential);
        unique_sessionhandle sessionHandle = ConvertToSessionHandle(credentialAndSessionInfo.sessionInfo);

        debug_print((L"DEBUG: load_user_bound_key - credential pointer: 0x" + std::to_wstring(credentialWrapper.get())).c_str());
        ...
        debug_print(L"DEBUG: load_user_bound_key - userboundkey_get_authorization_context_from_credential completed");
        debug_print((L"DEBUG: load_user_bound_key - authContextBlob size: " + std::to_wstring(authContextBlob.size())).c_str());

        // AUTH CONTEXT
        debug_print(L"DEBUG: load_user_bound_key - About to call GetUserBoundKeyAuthContext");
        unique_auth_context_handle authContext;
        THROW_IF_FAILED(GetUserBoundKeyAuthContext(
            sessionHandle.get(),
            authContextBlob.data(),
        ...
        debug_print(L"DEBUG: load_user_bound_key - GetUserBoundKeyAuthContext completed");

        // Validate
        debug_print(L"DEBUG: load_user_bound_key - About to validate auth context");
        USER_BOUND_KEY_AUTH_CONTEXT_PROPERTY propCacheConfig;
        propCacheConfig.name = UserBoundKeyAuthContextPropertyCacheConfig;
        propCacheConfig.size = sizeof(cacheConfig);
        propCacheConfig.value = &(const_cast<veil::vtl1::userboundkey::keyCredentialCacheConfig&>(cacheConfig));
        THROW_IF_FAILED(ValidateUserBoundKeyAuthContext(
            ...
            1,
            &propCacheConfig)); // OS CALL
        debug_print(L"DEBUG: load_user_bound_key - Auth context validation completed");
        ...
        debug_print(L"DEBUG: load_user_bound_key - About to call userboundkey_get_secret_from_credential");
        ...
        debug_print(L"DEBUG: load_user_bound_key - userboundkey_get_secret_from_credential completed");
        debug_print((L"DEBUG: load_user_bound_key - secret size: " + std::to_wstring(secret.size())).c_str());

        // DECRYPT USERKEY
        debug_print(L"DEBUG: load_user_bound_key - About to call UnprotectUserBoundKey");
        ...
        THROW_IF_FAILED(UnprotectUserBoundKey(
            ...
            &userkeySize)); // OS CALL
        debug_print(L"DEBUG: load_user_bound_key - UnprotectUserBoundKey completed");
        ...
        debug_print(L"DEBUG: load_user_bound_key - Function completed successfully");
    ...
    catch (const std::exception& e)
    {
        ...
        std::string error_msg = e.what();
        std::wstring werror_msg(error_msg.begin(), error_msg.end());
        debug_print((L"ERROR: load_user_bound_key - Exception caught: " + werror_msg).c_str());
        throw; // Re-throw the exception
    }
    catch (...)
    {
        debug_print(L"ERROR: load_user_bound_key - Unknown exception caught");
        throw; // Re-throw the exception
    }
}
...
std::vector<uint8_t> reseal_user_bound_key(
    ...
    uint32_t runtimePolicy)
{
    debug_print(L"DEBUG: reseal_user_bound_key - Function entered");
    debug_print((L"DEBUG: reseal_user_bound_key - boundKeyBytes size: " + std::to_wstring(boundKeyBytes.size())).c_str());
    debug_print((L"DEBUG: reseal_user_bound_key - sealingPolicy: " + std::to_wstring(static_cast<uint32_t>(sealingPolicy))).c_str());
    debug_print((L"DEBUG: reseal_user_bound_key - runtimePolicy: 0x" + std::to_wstring(runtimePolicy)).c_str());
    ...
    try
    {
        ...
        debug_print((L"DEBUG: reseal_user_bound_key - Successfully re-sealed bound key, new size: " + std::to_wstring(resealedData.size())).c_str());
        debug_print(L"DEBUG: reseal_user_bound_key - Function completed successfully");
    ...
    catch (const std::exception& e)
    {
        ...
        std::string error_msg = e.what();
        std::wstring werror_msg(error_msg.begin(), error_msg.end());
        debug_print((L"ERROR: reseal_user_bound_key - Exception caught: " + werror_msg).c_str());
        throw; // Re-throw the exception
    }
    catch (...)
    {
        debug_print(L"ERROR: reseal_user_bound_key - Unknown exception caught");
        throw; // Re-throw the exception
    }
}
} // namespace veil::vtl1::userboundkey
