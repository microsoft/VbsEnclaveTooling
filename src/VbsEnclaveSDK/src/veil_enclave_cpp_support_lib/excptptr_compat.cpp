// (ASG) Ported from https://github.com/microsoft/STL/blob/main/stl/src/excptptr.cpp
...
#include <ehdata.h>
#include <exception>
// #include <internal_shared.h> ASG Excluded
#include <malloc.h>
#include <memory>
#include <new>
#include <stdexcept>
#include "trnsctrl_compat.h" // ASG Modified
...
namespace
{
    ...
    struct _Constexpr_excptptr_immortalize_impl
    {
        ...
        constexpr _Constexpr_excptptr_immortalize_impl() noexcept
            : _Storage{}
        {
        }

        _Constexpr_excptptr_immortalize_impl(_Constexpr_excptptr_immortalize_impl const &) = delete;
        _Constexpr_excptptr_immortalize_impl & operator=(_Constexpr_excptptr_immortalize_impl const &) = delete;
    ...
    void _PopulateCppExceptionRecord(_EXCEPTION_RECORD & _Record, void const * const _PExcept, ThrowInfo * _PThrow) noexcept
    {
        ...
        if (_PThrow && (_PThrow->attributes & TI_IsWinRT))
        {
            ...
            auto const _PWei = (*static_cast<WINRTEXCEPTIONINFO ** const *>(_PExcept))[-1];
        ...
        void * _ThrowImageBase =
            _PThrow ? RtlPcToFileHeader(const_cast<void *>(static_cast<void const *>(_PThrow)), &_ThrowImageBase) : nullptr;
    ...
    void _CopyExceptionRecord(_EXCEPTION_RECORD & _Dest, const _EXCEPTION_RECORD & _Src) noexcept
    {
        ...
        auto const _Parameters = _Src.NumberParameters;
        ...
        auto const _In_use = (_STD min)(_Parameters, _Max_parameters);
    ...
    void _CopyExceptionObject(
        void * _Dest,
        void const * _Src,
        CatchableType const * const _PType
#if _EH_RELATIVE_TYPEINFO
        ,
        uintptr_t const _ThrowImageBase
#endif // _EH_RELATIVE_TYPEINFO
    )
    {
        ...
        if ((_PType->properties & CT_IsSimpleType) || _PType->copyFunction == 0)
        {
            ...
            {
                auto const _PUnknown = *static_cast<IUnknown * const *>(_Src);
                if (_PUnknown)
                {
                    _PUnknown->AddRef();
                }
...
#if _EH_RELATIVE_TYPEINFO
        auto const _CopyFunc = reinterpret_cast<void *>(_ThrowImageBase + _PType->copyFunction);
#else  // ^^^ _EH_RELATIVE_TYPEINFO / !_EH_RELATIVE_TYPEINFO vvv
        auto const _CopyFunc = _PType->copyFunction;
#endif // ^^^ !_EH_RELATIVE_TYPEINFO ^^^

        auto const _Adjusted = __AdjustPointer(const_cast<void *>(_Src), _PType->thisDisplacement);
...
static_assert(
    sizeof(exception_ptr) == sizeof(shared_ptr<const _EXCEPTION_RECORD>)
...
namespace
{
    ...
    public:
        ...
        explicit _ExceptionPtr_static() noexcept
            : _Ref_count_base()
        {
    ...
    private:
        ...
        void _Destroy() noexcept override
        {
            ...
            auto const & _CppEhRecord = reinterpret_cast<EHExceptionRecord &>(_ExRecord);
            ...
            auto const _PThrow = _CppEhRecord.params.pThrowInfo;
            if (!_PThrow)
            {
...
#if _EH_RELATIVE_TYPEINFO
            auto const _ThrowImageBase = reinterpret_cast<uintptr_t>(_CppEhRecord.params.pThrowImageBase);
            auto const _CatchableTypeArray = reinterpret_cast<CatchableTypeArray const *>(
                static_cast<uintptr_t>(_PThrow->pCatchableTypeArray) + _ThrowImageBase);
            auto const _PType = reinterpret_cast<CatchableType *>(
                static_cast<uintptr_t>(_CatchableTypeArray->arrayOfCatchableTypes[0]) + _ThrowImageBase);
#else  // ^^^ _EH_RELATIVE_TYPEINFO / !_EH_RELATIVE_TYPEINFO vvv
            auto const _PType = _PThrow->pCatchableTypeArray->arrayOfCatchableTypes[0];
#endif // ^^^ !_EH_RELATIVE_TYPEINFO ^^^
            ...
            {
                auto const _PUnknown = *static_cast<IUnknown * const *>(_CppEhRecord.params.pExceptionObject);
                if (_PUnknown)
                {
                    _PUnknown->Release();
                }
    ...
    public:
        ...
        explicit _ExceptionPtr_normal(const _EXCEPTION_RECORD & _Record) noexcept
            : _Ref_count_base()
        {
    ...
    static_assert(
        sizeof(_ExceptionPtr_normal) % __STDCPP_DEFAULT_NEW_ALIGNMENT__ == 0,
        "Exception in exception_ptr would be constructed with the wrong alignment");

    void _Assign_seh_exception_ptr_from_record(
        shared_ptr<const _EXCEPTION_RECORD> & _Dest,
        const _EXCEPTION_RECORD & _Record,
        void * const _RxRaw) noexcept
    {
        ...
        auto const _Rx = ::new (_RxRaw) _ExceptionPtr_normal(_Record);
        _Exception_ptr_access::_Set_ptr_rep(_Dest, &_Rx->_ExRecord, _Rx);
    }

    void _Assign_cpp_exception_ptr_from_record(shared_ptr<const _EXCEPTION_RECORD> & _Dest, EHExceptionRecord const & _Record) noexcept
    {
        ...
        //      bad_exception
        auto const _PThrow = _Record.params.pThrowInfo;
#if _EH_RELATIVE_TYPEINFO
        auto const _ThrowImageBase = reinterpret_cast<uintptr_t>(_Record.params.pThrowImageBase);
        auto const _CatchableTypeArray = reinterpret_cast<CatchableTypeArray const *>(
            static_cast<uintptr_t>(_PThrow->pCatchableTypeArray) + _ThrowImageBase);
        auto const _PType = reinterpret_cast<CatchableType *>(
            static_cast<uintptr_t>(_CatchableTypeArray->arrayOfCatchableTypes[0]) + _ThrowImageBase);
        ...
        auto const _ExceptionObjectSize = static_cast<size_t>(_PType->sizeOrOffset);
        auto const _AllocSize = sizeof(_ExceptionPtr_normal) + _ExceptionObjectSize;
        ...
        try
        {
            _CopyExceptionObject(
                ...
                _Record.params.pExceptionObject,
                _PType
#if _EH_RELATIVE_TYPEINFO
                ,
                _ThrowImageBase
            ...
            auto const _Rx = ::new (_RxRaw) _ExceptionPtr_normal(reinterpret_cast<const _EXCEPTION_RECORD &>(_Record));
            reinterpret_cast<EHExceptionRecord &>(_Rx->_ExRecord).params.pExceptionObject =
                static_cast<_ExceptionPtr_normal *>(_RxRaw) + 1;
            _Exception_ptr_access::_Set_ptr_rep(_Dest, &_Rx->_ExRecord, _Rx);
        }
        catch (...)
        {                                                    // copying the exception object threw an exception
            auto const & _InnerRecord = *_pCurrentException; // exception thrown by the original exception's copy ctor
            if (_InnerRecord.ExceptionCode == MANAGED_EXCEPTION_CODE || _InnerRecord.ExceptionCode == MANAGED_EXCEPTION_CODE_V4)
            {
            ...
            if (!PER_IS_MSVC_PURE_OR_NATIVE_EH(&_InnerRecord))
            { // catching a non-C++ exception depends on /EHa
                _Assign_seh_exception_ptr_from_record(_Dest, reinterpret_cast<const _EXCEPTION_RECORD &>(_InnerRecord), _RxRaw);
                return;
            }

            auto const _PInnerThrow = _InnerRecord.params.pThrowInfo;
#if _EH_RELATIVE_TYPEINFO
            auto const _InnerThrowImageBase = reinterpret_cast<uintptr_t>(_InnerRecord.params.pThrowImageBase);
            auto const _InnerCatchableTypeArray = reinterpret_cast<CatchableTypeArray const *>(
                static_cast<uintptr_t>(_PInnerThrow->pCatchableTypeArray) + _InnerThrowImageBase);
            auto const _PInnerType = reinterpret_cast<CatchableType *>(
                static_cast<uintptr_t>(_InnerCatchableTypeArray->arrayOfCatchableTypes[0]) + _InnerThrowImageBase);
#else  // ^^^ _EH_RELATIVE_TYPEINFO / !_EH_RELATIVE_TYPEINFO vvv
            auto const _PInnerType = _PInnerThrow->pCatchableTypeArray->arrayOfCatchableTypes[0];
#endif // ^^^ !_EH_RELATIVE_TYPEINFO ^^^

            auto const _InnerExceptionSize = static_cast<size_t>(_PInnerType->sizeOrOffset);
            auto const _InnerAllocSize = sizeof(_ExceptionPtr_normal) + _InnerExceptionSize;
            ...
            { // copying the exception emitted while copying the original exception also threw, give up
                free(_RxRaw);
                _Dest = _ExceptionPtr_static<bad_exception>::_Get();
                return;
            }
            ...
            auto const _Rx = ::new (_RxRaw) _ExceptionPtr_normal(reinterpret_cast<const _EXCEPTION_RECORD &>(_InnerRecord));
            reinterpret_cast<EHExceptionRecord &>(_Rx->_ExRecord).params.pExceptionObject =
                static_cast<_ExceptionPtr_normal *>(_RxRaw) + 1;
            _Exception_ptr_access::_Set_ptr_rep(_Dest, &_Rx->_ExRecord, _Rx);
        }
...
{
    ::new (_Dest) shared_ptr<const _EXCEPTION_RECORD>(*static_cast<shared_ptr<const _EXCEPTION_RECORD> const *>(_Src));
}
...
_CRTIMP2_PURE void __CLRCALL_PURE_OR_CDECL __ExceptionPtrAssign(_Inout_ void * _Dest, _In_ const void * _Src) noexcept
{
    *static_cast<shared_ptr<const _EXCEPTION_RECORD> *>(_Dest) =
        *static_cast<shared_ptr<const _EXCEPTION_RECORD> const *>(_Src);
}

_CRTIMP2_PURE bool __CLRCALL_PURE_OR_CDECL __ExceptionPtrCompare(_In_ const void * _Lhs, _In_ const void * _Rhs) noexcept
{
    return *static_cast<shared_ptr<const _EXCEPTION_RECORD> const *>(_Lhs)
        == *static_cast<shared_ptr<const _EXCEPTION_RECORD> const *>(_Rhs);
}

_CRTIMP2_PURE bool __CLRCALL_PURE_OR_CDECL __ExceptionPtrToBool(_In_ const void * _Ptr) noexcept
{
    return static_cast<bool>(*static_cast<shared_ptr<const _EXCEPTION_RECORD> const *>(_Ptr));
}

_CRTIMP2_PURE void __CLRCALL_PURE_OR_CDECL __ExceptionPtrSwap(_Inout_ void * _Lhs, _Inout_ void * _Rhs) noexcept
{
    static_cast<shared_ptr<const _EXCEPTION_RECORD> *>(_Lhs)->swap(*static_cast<shared_ptr<const _EXCEPTION_RECORD> *>(_Rhs));
}

_CRTIMP2_PURE void __CLRCALL_PURE_OR_CDECL __ExceptionPtrCurrentException(void * _Ptr) noexcept
{
    auto const _PRecord = _pCurrentException; // nontrivial FLS cost, pay it once
    if (!_PRecord || _PRecord->ExceptionCode == MANAGED_EXCEPTION_CODE || _PRecord->ExceptionCode == MANAGED_EXCEPTION_CODE_V4)
    {
    ...
    else
    {
        ...
        _Assign_seh_exception_ptr_from_record(
            _Dest,
            reinterpret_cast<_EXCEPTION_RECORD &>(*_PRecord),
            malloc(sizeof(_ExceptionPtr_normal)));
    }
}

[[noreturn]] _CRTIMP2_PURE void __CLRCALL_PURE_OR_CDECL __ExceptionPtrRethrow(_In_ const void * _PtrRaw)
{
    shared_ptr<const _EXCEPTION_RECORD> const * _Ptr = static_cast<shared_ptr<const _EXCEPTION_RECORD> const *>(_PtrRaw);
    ...
    if (PER_IS_MSVC_PURE_OR_NATIVE_EH(&_CppRecord))
    {
        ...
        auto const _PThrow = _CppRecord.params.pThrowInfo;
...
#if _EH_RELATIVE_TYPEINFO
        auto const _ThrowImageBase = reinterpret_cast<uintptr_t>(_CppRecord.params.pThrowImageBase);
        auto const _CatchableTypeArray =
            reinterpret_cast<CatchableTypeArray const *>(_ThrowImageBase + _PThrow->pCatchableTypeArray);
#else  // ^^^ _EH_RELATIVE_TYPEINFO / !_EH_RELATIVE_TYPEINFO vvv
        auto const _CatchableTypeArray = _PThrow->pCatchableTypeArray;
#endif // ^^^ !_EH_RELATIVE_TYPEINFO ^^^
...
#pragma warning(suppress : 6255) //  _alloca indicates failure by raising a stack overflow exception
    ...
    RaiseException(_RecordCopy.ExceptionCode, _RecordCopy.ExceptionFlags, _RecordCopy.NumberParameters, _RecordCopy.ExceptionInformation);
}
    ...
    __ExceptionPtrCopyException(_Inout_ void * _Ptr, _In_ const void * _PExceptRaw, _In_ const void * _PThrowRaw) noexcept
{
    ...
    _Assign_cpp_exception_ptr_from_record(
        *static_cast<shared_ptr<const _EXCEPTION_RECORD> *>(_Ptr),
        reinterpret_cast<EHExceptionRecord const &>(_Record));
}
