    ...
    namespace telemetry
    {
        ...
        class logger
        {
            ...
            logger(const std::wstring& providerName,
                ...
                const veil::any::telemetry::eventLevel level) : provider(providerName), guid(guidStr), logLevel(level)
            {
                SetLogFilePath();
            }

            void AddTimestampedLog(const std::wstring& log, const veil::any::telemetry::eventLevel level) // Called from Host
            {
                if (level <= logLevel)
                {
            ...
            // Getters
            veil::any::telemetry::eventLevel GetLogLevel()
            {
                return logLevel;
            }
