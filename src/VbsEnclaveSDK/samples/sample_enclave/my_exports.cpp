...
bool RunHelloSecuredEncryptionKeyExample_LoadEncryptionKeyImpl(_In_ sample::args::RunHelloSecuredEncryptionKeyExample_LoadEncryptionKey* data)
{
    ...
    //
    // Now let's test out the encryption key we just decrypted!
    //

    // A simple message to encrypt
    auto const SOME_PLAIN_TEXT = L"<____ Hi, ThIs Is SoMe PlAiN-tExT tO eNcRyPt ____>";
    ...
    // Let's decrypt a thing
    debug_print(L"4. Decrypting text...");
    auto decryptedText = veil::vtl1::crypto::decrypt(encryptionKey.get(), encryptedText, veil::vtl1::crypto::zero_nonce, tag);
    debug_print(L" ...CHECKPOINT: decrypted text: = %ws", veil::vtl1::to_wstring(decryptedText).c_str());
    debug_print("");
