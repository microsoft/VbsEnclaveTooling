...
namespace VbsEnclaveToolingTests
{
    ...
    TEST_CLASS(EdlParserImportTests)
    {
        private:
            ...
            std::filesystem::path m_edl_file_without_overlapped_imports = m_base_imports_path / "ImportTest.edl";
            std::filesystem::path m_edl_file_with_overlapped_imports = m_base_imports_path / "OverlappingImports" / "A_Overlap.edl";
                ...
                std::unordered_set<std::string>& m_dev_type_names)
            {
                ...
                Assert::AreEqual(trusted_function_list_size, edl.m_trusted_functions_list.size());
                Assert::AreEqual(untrusted_function_list_size, edl.m_untrusted_functions_list.size());

                auto check_functions_expected =
                    [&] (std::vector<std::string> func_list, const std::unordered_map<std::string, Function>& function_map)
                {
                    for (auto& function_signature : func_list)
                    {
                        auto function = function_map.at(function_signature);
                ...
                check_functions_expected(edl.m_trusted_functions_list, edl.m_trusted_functions_map);
                check_functions_expected(edl.m_untrusted_functions_list, edl.m_untrusted_functions_map);

                // Verify developer types were added to edl object
                Assert::AreEqual(m_dev_type_names.size(), edl.m_developer_types.size());
                Assert::AreEqual(m_dev_type_names.size(), edl.m_developer_types_list.size());
                for (auto& dev_type_name : edl.m_developer_types_list)
                {
        ...
        public:
            ...
            // Trusted functions
            TEST_METHOD(Parse_Edl_file_without_overlapping_imports)
            {
                std::unordered_set<std::string> m_dev_type_names =
                {
                    "__anonymous_enum",
                ...
                ParseEdlFileWithImports(
                    m_edl_file_without_overlapped_imports,
            ...
            TEST_METHOD(Parse_Edl_file_with_overlapping_imports)
            {
                std::unordered_set<std::string> m_dev_type_names =
                {
                    "__anonymous_enum",
                ...
                auto overlap_dir = m_base_imports_path / "OverlappingImports";
                auto directories = {std::filesystem::current_path(), overlap_dir};
                auto num_of_functions_to_parse = 4;// 4 trusted and 4 untrusted
                ParseEdlFileWithImports(
                    m_edl_file_with_overlapped_imports,
                    directories,
                    num_of_functions_to_parse,
                    num_of_functions_to_parse,
                    m_dev_type_names);
