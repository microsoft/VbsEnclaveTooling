...
The EDL contract models VTL0 as a bounded, non-blocking transport provider. The implemented `TlsTransport.edl` (ABI version 2) collapses the trusted surface to a single run-to-completion call — the enclave runs the whole exchange itself and returns only a derived result — plus a metadata query for display:

```text
trusted VTL0 -> VTL1:
  TlsSample_GetScenarioMetadata(...)
  TlsSample_RunScenario(...)
...
Earlier drafts modelled the trusted surface as a `StartScenario` / `DriveConnection` / `GetDerivedResult` / `CloseScenario` pump; that pump was collapsed into the single `TlsSample_RunScenario` export (see the `TlsSampleAbiVersion` comment in `SampleApps/Tls/TlsTransport.edl`).
